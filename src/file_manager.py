...
class FileManager(QTreeView):
    ...
    def show_context_menu(self, pos: QPoint):
        ...
        ix = self.indexAt(pos)
        menu = QMenu()
        menu.addAction("Nuevo archivo")
        menu.addAction("Nueva carpeta")
        menu.addAction("Abrir en el administrador de archivos")

        if ix.column() != -1:
            menu.addAction("Renombrear")
        ...
        if action.text() == "Renombrar":
            self.action_rename(ix)
        elif action.text() == "Eliminar":
            self.action_delete(ix)
        elif action.text() == "Nueva carpeta":
            self.action_new_folder()
        elif action.text() == "Nuevo archivo":
            self.action_new_file(ix)
        elif action.text() == "Abrir en el administrador de archivos":
    ...
    def action_delete(self, ix):
        ...
        dialog = self.show_dialog(
            "Eliminar", f"¿Estás seguro de que quieres eliminar {file_name}?"
        )
    ...
    def action_new_folder(self):
        f = Path(self.model.rootPath()) / "Nuevo archivo"
        count = 1
        while f.exists():
            f = Path(f.parent / f"Nueva carpeta{count}")
            count += 1
